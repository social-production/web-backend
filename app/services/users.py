...
def _get_settings_for_user(db: Session, user_id: UUID) -> Mapping[str, object]:
    row = (
        db.execute(select(user_settings).where(user_settings.c.user_id == user_id))
        .mappings()
        .first()
    )
    if row is None:
    ...
    user_row = (
        db.execute(
            select(
                ...
                users.c.profile_image_url,
                users.c.is_active,
            ).where(users.c.id == current_user_id)
        )
        .mappings()
        .first()
    )
...
def update_own_profile_settings(
    db: Session, current_user_id: UUID, payload: dict[str, object]
) -> dict[str, object]:
...
def accept_follow_request(
    db: Session, current_user_id: UUID, follower_username: str
) -> dict[str, object]:
    ...
    accepter_row = db.execute(select(users.c.username).where(users.c.id == current_user_id)).first()
